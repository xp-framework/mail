...
class HtmlMessage extends MimeMessage {
  ...
  public function __construct() {
    ...
    with ($multi= $this->addPart(new \peer\mail\MultiPart())); {
      $this->text= $multi->addPart(new \peer\mail\MimePart('', 'text/plain'));
      $this->text->setDisposition(MIME_DISPOSITION_INLINE);
      $this->html= $multi->addPart(new \peer\mail\MimePart('', 'text/html'));
  ...
  public function addImage($data, $contentType) {
    with ($image= $this->addPart(new \peer\mail\MimePart())); {
