...
class CclientStore extends MailStore {
  ...
  protected function _supports($u, &$attr) {
    throw new \lang\IllegalArgumentException('Scheme "'.$u->getScheme().'" not recognized');
  }
  ...
  public function hasFolder($name) {
    ...
    if (imap_last_error()) {
      throw new \peer\mail\MessagingException('Listing folders failed', $this->_errors());
    }
    return !empty($r);
  }
  ...
  public function getFolder($name) { 
    if (!$this->cache->has(SKEY_FOLDER.$name)) {
      if (false === imap_list($this->_hdl[0], $this->_hdl[1], $name)) {
        throw new \peer\mail\MessagingException('Retrieving folder "'.$name.'" failed', $this->_errors());
      }
  ...
  public function moveMessage($f, $msg) {
    if (false === imap_mail_move($this->_hdl[0], $msg->uid, $f->name, CP_UID)) {
      throw (new \peer\mail\MessagingException('Can not move mail'));
    }
